...
@IonicPage()
@Component({
  selector: 'page-slotinstallation',
  templateUrl: 'slotinstallation.html',
})
export class SlotinstallationPage {
  ...
  constructor(
    ...
    this.slotterpakai = this.calendar['slot_installation_total'] - this.calendar['slot_installation_available']
    platform.ready().then(() => {
  ...
  doGetSlot() {
    this.api.get('table/slot_installation', { params: { filter: "date_installation=" + "'" + this.calendar['fulldate'] + "'", sort: 'slot_no DESC' } })
      .subscribe(val => {
  ...
  getNextNo() {
    return this.api.get('nextno/slot_create_installation/slot_id_create')
  }
  doGetNextNo(data) {
  ...
  doAddCreateSlot(nextno, data) {
    ...
    this.api.post("table/slot_create_installation",
      {
        "slot_id_create": nextno,
        "total_slot": data.slot,
        "date_installation": this.calendar['fulldate'],
    ...
    this.api.put("table/calendar",
      {
        ...
        "slot_installation_total": parseInt(datacalendar.slot_installation_total) + parseInt(data.slot),
        "slot_installation_available": parseInt(datacalendar.slot_installation_available) + parseInt(data.slot)
      },
      { headers })
      .subscribe(
        (val) => {
  ...
  doCreateSlot(data) {
    const headers = new HttpHeaders()
      .set("Content-Type", "application/json");
    this.api.post("table/slot_installation",
      {
        "request_no": '',
        "location_request": '',
        "group_delivery_no": '',
  ...
  doGetSlotHistory() {
    this.api.get('table/slot_create_installation', { params: { limit: 100 ,filter: "date_installation=" + "'" + this.calendar['fulldate'] + "'", sort: 'date_installation DESC' } })
      .subscribe(val => {
  ...
  doGetSlotTerpakai() {
    this.api.get('table/slot_installation', { params: { limit: 1000 ,filter: "date_installation=" + "'" + this.calendar['fulldate'] + "' AND receipt_no != ''", sort: 'datetime DESC' } })
      .subscribe(val => {
