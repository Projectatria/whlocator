...
@IonicPage()
@Component({
  selector: 'page-scheduleinstallation',
  templateUrl: 'scheduleinstallation.html',
})
export class ScheduleinstallationPage {
  ...
  doGetSlot(date) {
    ...
    this.api.get("table/slot_installation", { params: { limit: 1000, filter: "date_installation =" + "'" + date.fulldate + "'", sort: 'receipt_no ASC' } })
      .subscribe(val => {
  ...
  doSlot(date) {
    this.navCtrl.push('SlotinstallationPage', {
      calendar: date
    })
  }
  slideChanged() {
  ...
  doGetDeliveryOrder() {
    this.api.get("table/delivery_order_header", { params: { limit: 100, filter: "store_no=" + "'" + this.rolecab + "' AND (installation_booking= '' OR installation_booking IS NULL)", sort: 'receipt_date ASC' } })
      .subscribe(val => {
  ...
  getSearch(ev: any) {
    ...
    if (value && value.trim() != '') {
      this.api.get("table/delivery_order_header", { params: { limit: 10, filter: "store_no=" + "'" + this.rolecab + "' AND receipt_no LIKE" + "'%" + value + "%' AND (installation_booking= '' OR installation_booking IS NULL)", sort: 'receipt_date ASC' } })
        .subscribe(val => {
          let data = val['data']
  ...
  doUpdateDeliveryOrderHeader(dod, nextno) {
    ...
    this.api.put("table/delivery_order_header",
      {
        "uuid": dod.uuid,
        "installation_date": this.dateselect['fulldate'],
        "installation_booking": nextno
      },
      { headers })
      .subscribe(
        (val) => {
  ...
  doGetCalendar() {
    this.api.get("table/calendar", { params: { limit: 10, filter: "fulldate=" + "'" + this.dateselect['fulldate'] + "'" } })
      .subscribe(val => {
        let data = val['data']
        let slotavailable = data[0].slot_installation_available
    ...
    this.api.put("table/calendar",
      {
        ...
        "slot_installation_available": parseInt(slotavailable) - 1
      },
      { headers })
      .subscribe(
        (val) => {
